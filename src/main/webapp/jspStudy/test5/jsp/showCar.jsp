...
<%@ page import="jspStudy.test5.ShopCar" %>
<head>
         <title>购物车页面</title>
         <link type="text/css" href="../../test3/css/myCart.css" rel="stylesheet"/>
...
<body>
 ...
 <%
     ...
     ShopCar shopCar=(ShopCar) session.getAttribute("myCar");
   if (shopCar == null|| shopCar.getSize()== 0) {
       %>
              <tr height="100">
                  ...
                  <%} else{
                     Book book;
                     for(HashMap.Entry<String, Book> entry : shopCar.getBuylist().entrySet()){
              ...
              <tr class="tableboder">
                  ...
                  <td class="cart_td_5">                      <!--    minus1001,minus1002 ,num1001, -->
                      ...
                      <input name="num<%=book.getBookNo() %>" id="num<%=book.getBookNo() %>" type="text" value="<%=book.getBuyNum()%>" class="num_input" readonly="readonly" />
                      <img src="../../images/add.jpg" class="hand" id="add<%=book.getBookNo()%>" onclick="clickadd(<%=book.getBookNo()%>,<%=book.getNowPrice()%>)"  />
                  </td>
                  <td class="cart_td_6" id="total<%=book.getBookNo() %>"><%=book.getBuyNum()*book.getNowPrice()%>元</td>
                  <td class="cart_td_7"><a href="../../../DoCarServlet?action=removeone&bookNo=<%=entry.getKey() %>">删除</a></td>
              </tr>
              <%}
              }
              %>
